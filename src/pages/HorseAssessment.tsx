...
export function HorseAssessment() {
  ...
  return <div className="mx-auto max-w-3xl px-4 py-14 sm:px-6">
    ...
    <form onSubmit={submit} className="mt-8 space-y-5 rounded-lg border border-hunter-800/10 bg-oat-50 p-6 sm:p-8">
      ...
      <div className="grid gap-5 sm:grid-cols-2"><label><span className="font-display text-sm font-semibold">Activité *</span><select required name="activity" className={input}><option value="">Sélectionnez</option><option>Loisir</option><option>Élevage</option><option>Retraité</option></select></label><label><span className="font-display text-sm font-semibold">Besoin principal *</span><select required name="need" className={input}><option value="">Sélectionnez</option><option>Alimentation / état corporel</option><option>Digestion</option><option>Articulations</option><option>Sabots</option><option>Électrolytes</option><option>Autre</option></select></label></div>
